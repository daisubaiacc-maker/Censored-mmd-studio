...
viewport.onMeshSelected = (mesh, hitPoint) => {
  ...
  if (!root) return;
  const region: CensorshipRegion = { id: `censor-${crypto.randomUUID()}`, space: 'screen', shape: 'rectangle', orientation: 'billboard', sizeMode: 'screen', x: 0, y: 0, width: 0, height: 0, effect: 'mosaic', enabled: true, pixelSize: Number(censorshipPixelSize.value), binding: { modelId: root.id, objectName: mesh.name } };
  censorshipBindings.bind(region, mesh, hitPoint); censorshipSelectionMode = false; viewport.setCensorshipSelectionMode(false); censorshipEditButton.textContent = '検閲対象を選択'; censorshipStatus.textContent = `登録: ${mesh.name || '(名称なし)'} / Rectangle Billboard / Screen`;
};

function updateCensorshipOptions(): void { for (const region of censorshipBindings.getRegions()) { region.sizeMode = censorshipSizeMode.value as 'screen' | 'world'; region.pixelSize = Number(censorshipPixelSize.value); } }
