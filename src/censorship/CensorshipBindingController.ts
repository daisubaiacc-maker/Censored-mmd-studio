...
export class CensorshipBindingController {
  ...
  getScreenRect(region: CensorshipRegion, camera: THREE.Camera, width: number, height: number): { x: number; y: number; width: number; height: number } | null {
    ...
    binding.object3D.updateWorldMatrix(true, false); binding.object3D.updateBillboard(camera);
  ...
  update(camera: THREE.Camera, width: number, height: number): void {
    ...
    for (const binding of this.bindings) {
      const region = binding.region; const model = region.model; if (!model || !region.enabled) continue;
      binding.object3D.position.copy(binding.localPoint).add(new THREE.Vector3(...model.position)); binding.object3D.rotation.set(...model.rotation); binding.object3D.setSize(model.width, model.height); binding.object3D.setBillboard(model.billboard); binding.object3D.updateBillboard(camera);
