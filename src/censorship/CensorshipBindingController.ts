...
export class CensorshipBindingController {
  ...
  bind(region: CensorshipRegion, target: THREE.Object3D, worldPoint?: THREE.Vector3): void {
    ...
    object3D.position.copy(localPoint).add(new THREE.Vector3(...model.position)); object3D.rotation.set(...model.rotation); object3D.setBillboard(model.billboard); target.add(object3D);
    this.bindings.push({ region, target, localPoint, object3D }); this.syncRegions();
  }

  addScreenRegion(region: CensorshipRegion): void { if (region.space !== 'screen' || !region.screen) throw new Error('Screen censorship regions require screen coordinates.'); this.screenRegions.push(region); this.syncRegions(); }
  ...
  getScreenRect(region: CensorshipRegion, camera: THREE.Camera, width: number, height: number): { x: number; y: number; width: number; height: number } | null {
    ...
    const binding = this.bindings.find((entry) => entry.region === region); if (!binding || !region.model) return null;
    this.syncObject(binding, camera);
    const projected = binding.object3D.getWorldCorners().map((corner) => corner.project(camera)); if (!projected.some((point) => point.z >= -1 && point.z <= 1)) return null;
    const xs = projected.map((point) => (point.x * 0.5 + 0.5) * width); const ys = projected.map((point) => (1 - (point.y * 0.5 + 0.5)) * height);
  ...
  moveScreenProjected(region: CensorshipRegion, camera: THREE.Camera, dx: number, dy: number, width: number, height: number): void {
    ...
    const plane = new THREE.Plane().setFromNormalAndCoplanarPoint(forward, origin); const a = this.rayPoint(camera, width, height, 0, 0, plane); const b = this.rayPoint(camera, width, height, dx, dy, plane); if (!a || !b) return;
  ...
  rotateProjected(region: CensorshipRegion, camera: THREE.Camera, pointerX: number, pointerY: number, width: number, height: number, startAngle: number, startRotation: number): void {
    ...
    let delta = angle - startAngle; while (delta > Math.PI) delta -= Math.PI * 2; while (delta < -Math.PI) delta += Math.PI * 2;
    region.model.rotation[2] = startRotation + delta;
  }

  setBillboard(region: CensorshipRegion, enabled: boolean): void {
    if (region.space !== 'model' || !region.model) return;
    region.model.billboard = enabled; const binding = this.bindings.find((entry) => entry.region === region); if (binding) binding.object3D.setBillboard(enabled);
  }
  ...
  update(camera: THREE.Camera, width: number, height: number): void {
    ...
    for (const binding of this.bindings) {
      const region = binding.region; if (!region.model || !region.enabled) continue; this.syncObject(binding, camera);
  ...
  private syncObject(binding: BoundRegion, camera: THREE.Camera): void {
    ...
    binding.object3D.position.copy(binding.localPoint).add(new THREE.Vector3(...model.position)); binding.object3D.rotation.set(...model.rotation); binding.object3D.setSize(model.width, model.height); binding.object3D.setBillboard(model.billboard); binding.object3D.updateBillboard(camera, model.rotation[2]);
  }
