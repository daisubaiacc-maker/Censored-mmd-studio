...
/** Desktop orbit/pan/zoom plus single-touch orbit. Zoom is exposed as an explicit UI control on touch devices. */
export class StudioCameraController {
  ...
  /** Set zoom from a normalized slider value: 0 = far, 1 = near. */
  setZoomNormalized(value: number): void {
    const normalized = THREE.MathUtils.clamp(value, 0, 1);
    const minRadius = 0.2;
    const maxRadius = 100;
    this.spherical.setFromVector3(this.camera.position.clone().sub(this.target));
    const radius = maxRadius * Math.pow(minRadius / maxRadius, normalized);
  ...
  getZoomNormalized(): number {
    ...
    return THREE.MathUtils.clamp(
      Math.log(maxRadius / this.spherical.radius) / Math.log(maxRadius / minRadius),
      0,
      1,
    );
  }
  ...
  private onPointerDown = (event: PointerEvent): void => {
    ...
    if (event.pointerType === 'touch') {
      // Only one-finger orbit belongs to the camera. Multi-touch is deliberately ignored
      // so that pinch/pan cannot compete with TransformControls or the browser.
  ...
  private onPointerMove = (event: PointerEvent): void => {
    if (!this.enabled || this.activeButton === null) return;
    if (event.pointerType === 'touch') {
      event.preventDefault();
  ...
  private onPointerUp = (event: PointerEvent): void => {
    ...
    if (event.pointerType === 'touch' || event.button === this.activeButton) {
      this.activeButton = null;
      this.panMode = false;
